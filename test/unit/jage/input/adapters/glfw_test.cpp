...
constexpr auto key_params = std::to_array<key_param>({
    ...
    make_key_param(GLFW_KEY_TAB, keyboard::key::tab, keyboard::scancode::tab),
    ...
    make_key_param(GLFW_KEY_END, keyboard::key::end, keyboard::scancode::end),
    ...
    make_key_param(GLFW_KEY_F10, keyboard::key::F10, keyboard::scancode::F10),
    make_key_param(GLFW_KEY_F11, keyboard::key::F11, keyboard::scancode::F11),
    make_key_param(GLFW_KEY_F12, keyboard::key::F12, keyboard::scancode::F12),
    make_key_param(GLFW_KEY_F13, keyboard::key::F13, keyboard::scancode::F13),
    make_key_param(GLFW_KEY_F14, keyboard::key::F14, keyboard::scancode::F14),
    make_key_param(GLFW_KEY_F15, keyboard::key::F15, keyboard::scancode::F15),
    make_key_param(GLFW_KEY_F16, keyboard::key::F16, keyboard::scancode::F16),
    make_key_param(GLFW_KEY_F17, keyboard::key::F17, keyboard::scancode::F17),
    make_key_param(GLFW_KEY_F18, keyboard::key::F18, keyboard::scancode::F18),
    make_key_param(GLFW_KEY_F19, keyboard::key::F19, keyboard::scancode::F19),
    make_key_param(GLFW_KEY_F20, keyboard::key::F20, keyboard::scancode::F20),
    make_key_param(GLFW_KEY_F21, keyboard::key::F21, keyboard::scancode::F21),
    make_key_param(GLFW_KEY_F22, keyboard::key::F22, keyboard::scancode::F22),
    make_key_param(GLFW_KEY_F23, keyboard::key::F23, keyboard::scancode::F23),
    make_key_param(GLFW_KEY_F24, keyboard::key::F24, keyboard::scancode::F24),
...
TEST_F(glfw_adapter, should_map_modifiers) {
                                ...
                                GLFW_MOD_CONTROL);
  {
    ...
    const auto event = std::get<keyboard_event>(context.buffer.front());
    EXPECT_TRUE(event.modifiers.test(static_cast<std::size_t>(
        std::to_underlying(keyboard::modifier::left_control))))
        << "Modifiers: " << event.modifiers.to_string();
    EXPECT_TRUE(event.modifiers.test(static_cast<std::size_t>(
        std::to_underlying(keyboard::modifier::right_control))))
        << "Modifiers: " << event.modifiers.to_string();
  }
  context.buffer.clear();
  platform.trigger_key_callback(GLFW_KEY_ESCAPE, 0x04, GLFW_PRESS,
                                GLFW_MOD_SUPER);
  {
    ...
    const auto event = std::get<keyboard_event>(context.buffer.front());
    EXPECT_TRUE(event.modifiers.test(static_cast<std::size_t>(
        std::to_underlying(keyboard::modifier::left_gui))))
        << "Modifiers: " << event.modifiers.to_string();
    EXPECT_TRUE(event.modifiers.test(static_cast<std::size_t>(
        std::to_underlying(keyboard::modifier::right_gui))))
        << "Modifiers: " << event.modifiers.to_string();
  }
}
...
TEST_F(glfw_adapter, should_set_timestamp) {
                                ...
                                GLFW_MOD_SHIFT);
  {
    ...
    ASSERT_TRUE(std::holds_alternative<keyboard_event>(context.buffer.front()));
    const auto event = std::get<keyboard_event>(context.buffer.front());
    EXPECT_EQ(context_type::duration_type{42e+9}, event.timestamp);
  }
  context.buffer.clear();
                                ...
                                GLFW_MOD_SHIFT);
  {
    ...
    ASSERT_TRUE(std::holds_alternative<keyboard_event>(context.buffer.front()));
    const auto event = std::get<keyboard_event>(context.buffer.front());
    EXPECT_EQ(context_type::duration_type{99e+9}, event.timestamp);
  }
}
