...
TEST_F(glfw_context, should_preserve_payload_on_rvalue_push) {
  context.push(event_type{duration_type{42}, scroll_type{1.5}});
  ASSERT_TRUE(std::holds_alternative<scroll_type>(sink.events.front().payload));
  EXPECT_DOUBLE_EQ(1.5,
                   std::get<scroll_type>(sink.events.front().payload).offset);
}
...
TEST_F(glfw_context, should_reflect_cursor_mutation_through_const_accessor) {
