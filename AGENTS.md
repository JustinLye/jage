...
- All builds, tests, and CMake commands must be run inside the devcontainer. Use `docker exec <container_name> bash -c "cd /workspaces/jage && <command>"` to run commands from the host. Never install build tools on the host machine.
