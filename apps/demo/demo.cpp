...
auto main(int, char *[]) -> int {
  ...
  auto monitor = platform.get_primary_monitor();
  const auto video_mode = platform.get_video_mode(monitor);
  platform.set_window_hint(GLFW_RED_BITS, video_mode->redBits);
  platform.set_window_hint(GLFW_BLUE_BITS, video_mode->blueBits);
  platform.set_window_hint(GLFW_GREEN_BITS, video_mode->greenBits);
  platform.set_window_hint(GLFW_REFRESH_RATE, video_mode->refreshRate);
  auto window = platform.create_window(video_mode->width, video_mode->height,
                                       "JAGE Demo", monitor, nullptr);
